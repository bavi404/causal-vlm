...
class AudioCapsDataset(Dataset):
    ...
    Note: AudioCaps typically has captions instead of Q&A pairs.
    This loader treats captions as "questions" and can use empty answers
    or the same caption as the answer.
    """
    ...
    def __init__(
        ...
        caption_key: Optional[str] = "caption",
    ):
        """
        Initialize AudioCaps dataset.
        ...
        Args:
            ...
            question_key: Key/column name for questions (or captions)
            answer_key: Key/column name for answers (optional, can use caption)
            caption_key: Alternative key for captions (if different from question_key)
        """
        ...
        # Process annotations (handle caption fallback)
        ...
        if suffix == '.json':
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                if isinstance(data, dict):
                    ...
                    for key in ['data', 'annotations', 'items', 'captions']:
                        if key in data:
                            return data[key]
    ...
    def _process_annotations(self) -> List[Dict]:
        """Process annotations to handle caption fallback for answers."""
        processed = []
        for ann in self.annotations:
            # If answer is missing but caption exists, use caption as answer
            if self.answer_key not in ann or not ann[self.answer_key]:
                if self.caption_key and self.caption_key in ann:
                    ann[self.answer_key] = ann[self.caption_key]
                elif self.question_key in ann:
                    # Use question/caption as answer if no separate answer
                    ann[self.answer_key] = ann[self.question_key]
            
            # If question is missing but caption exists, use caption as question
            if self.question_key not in ann or not ann[self.question_key]:
                if self.caption_key and self.caption_key in ann:
                    ann[self.question_key] = ann[self.caption_key]
    ...
    def __getitem__(self, idx: int) -> Dict[str, Union[str, List[str]]]:
        ...
        # Image is optional
        if self.image_key in entry and entry[self.image_key]:
        ...
        # Answer is optional (can be empty string)
        answer = entry.get(self.answer_key, "")
...
def compute_and_save_embeddings(
    ...
    Args:
        dataset: AudioCapsDataset instance
